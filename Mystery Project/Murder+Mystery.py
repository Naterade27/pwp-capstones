...
def get_average_sentence_length(text):
    ...
    # replace !, ?, and takes away extra space after sentence end
    plaintext = text.replace('! ', '.')
    almostplain = plaintext.replace('? ', '.')
    moreplain = almostplain.replace('. ', '.')
    splittext = moreplain.split('.')
   
    wordcount = 0 # initialize wordcounter
    
    for sent in splittext:
        wordcount += len(sent.split(' '))
        
    return wordcount / len(splittext)
...
class TextSample:
    ...
    def __repr__(self):
        return self.author + '\'s average length is ' + str(self.average_sentence_length)
...
def prepare_text(text):
    small = text.lower()
    #could turn this into a for loop, but meh
    noques = small.replace('?', '')
    noex = noques.replace('!', '')
    nocomma = noex.replace(',', '')
    noquote = nocomma.replace('\'', '')
    nofullquote = noquote.replace('\"', '')
    noperiod = nofullquote.replace('.', '')
    textwhite = noperiod.split(' ')
    return textwhite
...
def ngram_creator(text_list):
    ...
    for i in range(len(text_list)-1):
        ngramlist.append(text_list[i] + ' ' + text_list[i+1])
    return ngramlist
...
def frequency_comparison(table1, table2):
    ...
    for key, value in table1.items():
        if key in table2:
            ...
            appearances += (value)
        table1_keys.append(key)
        
    for key2, value2 in table2.items():
        if not key2 in table1_keys:
            appearances += (value2)
...
print(gregg_sample, find_text_similarity(gregg_sample, murderer_sample))
print(myrtle_sample, find_text_similarity(myrtle_sample, murderer_sample))
print(lily_sample, find_text_similarity(lily_sample, murderer_sample))
